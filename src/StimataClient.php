...
class StimataClient
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = rtrim($config['base_url'] ?? 'http://localhost:9091/api', '/');
    ...
    public function handleCallback($currentUrl, $expectedState)
    {
        ...
        if (isset($query['error'])) {
            throw new Exception("OAuth Error: " . ($query['error_description'] ?? $query['error']));
        }
    ...
    public function getUser($accessToken)
    {
        ...
        if (empty($response['success']) || empty($response['data'])) {
            throw new Exception($response['error'] ?? 'Failed to get user profile');
        }
    ...
    public function switchRole($accessToken, $role)
    {
        ...
        if ($httpCode >= 400) {
            $errorMsg = $data['error_description'] ?? $data['error'] ?? 'Request failed with status ' . $httpCode;
    ...
    {
        $data = $this->request('GET', '/v1/check-access', ['resource' => $resource], [
            'Authorization: Bearer ' . $accessToken
        ]);
